...
- `units`: 部队编制列表，从文本中识别出的独立军事部队。每个部队 MUST 包含名称（`name`）、所属阵营（`faction`）、指挥官（`commander`）、兵种类型（`troop_type`: infantry / cavalry / mixed）和兵力描述（`troop_count`）

LLM 提示词 MUST 不包含 `events`、`unit_states`、`units.direction`、`scale` 字段及对应提取规则。

系统 MUST 能区分文本中的实际军事行动和人物对话/议论内容。人物言论中假设、建议或讨论的军事行动（如「臣以为应从X出兵」）MUST NOT 被视为实际行军节点。朝堂对话和场景描写段落 MUST 被忽略，仅从军事行动描述段落中提取信息。部队实体 MUST 仅来源于实际军事行动中的部队描述。
