...
定义 ShaosongMap 项目的代码组织方式：`routers → services → schemas` 三层架构，确保关注点分离、可测试性和可维护性。
...
- **WHEN** 开发者编写业务逻辑单元测试
- **THEN** 可以直接 `from shaosongmap.services.geojson import make_geojson` 测试，无需启动 FastAPI 应用
...
- yield `PipelineStage` 对象（包含 `event` 和 `data` 字段）

路由层 MUST：
- 遍历 generator 产出的 `PipelineStage`
- 调用 `sse_event()` 转为 SSE 格式字符串
...
本次重构 MUST 不修改任何 API 端点路径、请求/响应格式、函数签名或业务逻辑。所有现有测试 SHALL 在重构前后保持通过。

#### Scenario: 重构后测试全绿

- **WHEN** 分层架构重构完成
- **THEN** 运行 `pytest tests/ -v` 全部测试通过，无任何修改
