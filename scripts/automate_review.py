...
def main() -> None:
    ...
    parser.add_argument(
        'text', nargs='?', default=_TEST_TEXT, help='战役文本（可选，有默认战术级文本）'
    )
    parser.add_argument('--output', '-o', default=None, help='审查报告输出文件（默认打印到终端）')
    parser.add_argument(
        '--headless', default='true', choices=['true', 'false'], help='是否无头模式（默认 true）'
    )
    ...
    with sync_playwright() as p:
        ...
        page.wait_for_function(
            "() => document.querySelector('#map canvas') !== null", timeout=15000
        )
        ...
        error_visible = page.evaluate(
            "() => document.getElementById('error-toast').classList.contains('show')"
        )
        if error_visible:
