...
DATASET_API = f"https://dataverse.harvard.edu/api/datasets/:persistentId/?persistentId={DATAVERSE_DOI}"
...
def _find_column(headers: list[str], *candidates: str) -> Optional[str]:
...
def _parse_zip(content: bytes, zip_name: str) -> tuple[str, list[str], list[dict]]:
    ...
    with ZipFile(BytesIO(content)) as zf:
        ...
        candidates = [
            ...
            if name.lower().endswith((".tsv", ".csv", ".tab"))
        ]
        if not candidates:
            raise RuntimeError(
                f"Zip 文件中未找到 Shapefile 或 TSV/CSV 文件。内容: {namelist}"
            )
...
def filter_song_dynasty(rows: list[dict], headers: list[str]) -> list[dict]:
    ...
    print(f"  列映射: name={name_col}, x={x_col}, y={y_col}, beg={beg_col}, end={end_col}, lev={lev_col or '(无)'}")

    stats = {"total": len(rows), "no_time": 0, "time_mismatch": 0, "no_coord": 0, "wrong_level": 0, "kept": 0}
    result = []
    ...
    print(f"  统计: 总计 {stats['total']}, 无时间 {stats['no_time']}, "
...
def main():
    ...
    try:
        ...
        print("提示: 也可以手动下载 CHGIS v6 数据后运行: python scripts/build_chgis.py <文件路径>",
              file=sys.stderr)
        sys.exit(1)
    ...
    if len(filtered) < 200:
        print(f"  ⚠ 警告: 仅筛选出 {len(filtered)} 条记录，可能筛选条件过严或数据列映射有误",
              file=sys.stderr)
