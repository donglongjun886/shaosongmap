...
7. units提取规则：从文本中识别独立军事部队实体，同一阵营下可能有多个独立行动的部队（如宋军的「焦文通部」「郦琼部」「秦凤路兵马」是三个不同部队）。部队名在全文JSON中保持统一，不要用别名。troop_type根据文本判断：提及骑/铁浮屠/合扎猛安/拐子马→cavalry，提及步/弩手/刀斧手→infantry，无法判断→mixed。direction用方位词（东/南/西/北/东南/西南/东北/西北），仅当文本明确描述进攻方向时填写
8. unit_states提取规则：为每个部队在有行为描述的步骤中记录状态快照。不是每个部队每个步骤都要有记录。status判断标准：部队列阵/驻扎/原地待命→deploying，部队向某方向移动/进军→marching，部队与敌军交战/接战→engaging，部队主动撤退/后撤→retreating，部队被击溃/全军覆没/崩溃→routing。location必须是places中已有地名。description用一句话描述该部队在此步骤的战术动作"""


def _validate_unit_states(unit_states: list[dict], units: list[dict], events: list[dict]) -> list[dict]:
    ...
    for unit in units:
        ...
        for existing in kept:
            ...
            if len(name) >= 3 and len(existing_name) >= 3:
                ...
                if dist <= 1:
                    logger.info("合并部队名变体: '%s' → '%s' (编辑距离=%d)", name, existing_name, dist)
                    is_dup = True
                    break
        if not is_dup:
            kept.append(unit)
