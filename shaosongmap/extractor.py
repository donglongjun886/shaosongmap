...
7. units提取规则：从文本中识别独立军事部队实体，同一阵营下可能有多个独立行动的部队（如宋军的「焦文通部」「郦琼部」「秦凤路兵马」是三个不同部队）。部队名在全文JSON中保持统一，不要用别名。troop_type根据文本判断：提及骑/铁浮屠/合扎猛安/拐子马→cavalry，提及步/弩手/刀斧手→infantry，无法判断→mixed。direction必须是标准八方位词之一：东/南/西/北/东南/西南/东北/西北。优先使用文本明确提及的方位；若未明确但可根据战术语境推断（如「侧翼压上」→攻击方相对防守方的侧面方向，「沿X地冲击」→从X地向目标地的方向），必须将其转换为标准方位词填入。绝不可填地名或非方位词（如「侧翼」「塬底」是错误的）。无法推断时填null
