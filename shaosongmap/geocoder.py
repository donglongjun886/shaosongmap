...
以 JSON 对象输出，格式为：
{{"results": [{{"name": "地名", "lng": 经度, "lat": 纬度, "confidence": "high|medium|low", "reasoning": "推断依据"}}]}}
            ...
            records.append(
                {
                    ...
                    'lng': float(row.get('x_coord') or 0),
...
def match_chgis(
    ...
    # 如果有朝代限制，按时间重叠过滤
    if dynasty_beg_yr is not None and dynasty_end_yr is not None:
...
def geocode(
    ...
    # 一次性检查 CHGIS 是否可用（lru_cache 不缓存异常，避免循环重复抛异常）
    try:
        _load_chgis_data()
        chgis_available = True
    except FileNotFoundError:
        logger.warning('CHGIS 数据文件未找到，将使用 LLM 推断')
