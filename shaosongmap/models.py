...
class CampaignExtract(BaseModel):
    ...
    scale: MilitaryScale | None = Field(
        default=None,
...
class TimelineEvent(BaseModel):
    ...
    places_involved: list[str] = Field(
        default_factory=list, description='事件涉及的地名，须为 places 中的已有地名'
    )
...
class ForceUnit(BaseModel):
    ...
    troop_type: TroopType = Field(
        default='mixed', description='兵种类型：infantry(步兵) / cavalry(骑兵) / mixed(混合)'
    )
...
class CampaignTimeline(CampaignExtract):
    ...
    events: list[TimelineEvent] = Field(
        default_factory=list, description='按时间顺序排列的事件序列'
    )
    units: list[ForceUnit] = Field(default_factory=list, description='识别出的部队实体列表')
    unit_states: list[UnitState] = Field(
        default_factory=list, description='各部队在各步骤中的状态快照'
    )
