...
from shaosongmap.models import CampaignTimeline, ForceUnit, UnitState, UnitStatus, TroopType
...
def test_campaign_timeline_with_units():
    ...
    ct = CampaignTimeline.model_validate({
        ...
        "routes": [],
        "events": [
            {"seq": 1, "event_type": "battle", "description": "交战", "actors": [], "places_involved": ["东坡塬"]}
        ],
        "units": [
            {"name": "焦文通部", "faction": "宋军", "commander": "焦文通", "troop_type": "mixed", "troop_count": "数千"}
        ],
        "unit_states": [
            {"seq": 1, "unit_name": "焦文通部", "status": "engaging", "location": "东坡塬", "description": "侧翼包抄"}
        ],
    })
...
def test_extract_timeline_with_units(mock_openai: MagicMock):
    ...
    mock_openai.return_value.chat.completions.create.return_value = _mock_response({
        ...
        "routes": [],
        "events": [
            {"seq": 1, "event_type": "march", "description": "出发", "actors": ["岳飞"], "places_involved": ["襄阳"]},
            {"seq": 2, "event_type": "battle", "description": "焦文通部侧翼包抄", "actors": ["焦文通"], "places_involved": ["东坡塬"]},
            {"seq": 3, "event_type": "battle", "description": "合扎猛安冲击，焦文通部溃散", "actors": ["焦文通", "合扎猛安"], "places_involved": ["东坡塬"]},
        ],
        "units": [
            {"name": "焦文通部", "faction": "宋军", "commander": "焦文通", "troop_type": "mixed", "troop_count": "数千", "direction": "东南"},
            {"name": "合扎猛安", "faction": "金军", "commander": "", "troop_type": "cavalry", "troop_count": "一千骑", "direction": "西北"},
        ],
        "unit_states": [
            {"seq": 2, "unit_name": "焦文通部", "status": "marching", "location": "东坡塬", "description": "侧翼包抄"},
            {"seq": 3, "unit_name": "焦文通部", "status": "routing", "location": "东坡塬", "description": "全军崩溃"},
            {"seq": 3, "unit_name": "合扎猛安", "status": "marching", "location": "东坡塬", "description": "铁骑冲击"},
        ],
        "scale": "tactical",
    })
